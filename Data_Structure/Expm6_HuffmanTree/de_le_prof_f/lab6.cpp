...
#define n 256 // the number of leadf nodes
// 定义结点总数，2n-1
#define m 511 // the number of nodes

// 定义树结点，包括三个指针父节点，左右孩子
typedef struct
{
	unsigned char id;
	long w; // weight of the vector
	int parent, lchild, rchild;
} HTNode;

// Huffman Tree Encoding info table
typedef struct
{
...
{
	long allw = 0;	 // all weights' sum
	int numZero = 0; // the number of not appearing characters
	for (int i = 0; i < m; ++i)
	{
		ht[i].id = i;									 // 编号
		ht[i].parent = ht[i].lchild = ht[i].rchild = -1; // 置空
		ht[i].w = 0;									 // 置零
		if (i < n)
		{
			ht[i].w = w[i];
...
void getCode(HTNode ht[], int c, unsigned *code1, int *clen1)
{
	...
	long long size = sizeof(code);			 // 编码的最大bits数
...
int main()
{
	...
	saveTree(ht,root,"pdftree.html"); //图形展示哈夫曼树

	// 编码，压缩文件,写入文件 fout
	char zfile[256] = "mytest.lzip";
	zip(zfile, ht, root, hc, content, fsize);

	/*************************** 以下为解码内容 ****************************/
	char ofile[256] = "nlab6.pdf";
	unzip(zfile, ofile);
