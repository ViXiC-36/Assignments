...
/*Shortcomings appear when the length is not so big
Under certain circumstances it might be blocked on a inferior peak*/
...
int main()
{
    ...
    pdiag = (int *)malloc(sizeof(int) * (2 * len - 1));
    cdiag = (int *)malloc(sizeof(int) * (2 * len - 1));
...
void getdiag()
{
    ...
    for (i = 0; i < len; i++)
    {
        pdiag[(len - 1) + (i - solution[i])]++;
        cdiag[i + solution[i]]++; // get the number of the queens of every diag line
    }
}
