...
#define leafNum 256 // the number of leadf nodes
// 定义结点总数，2n-1
...
int CreateHTree(HTNode ht[], long *w)
{
    ...
    for (int i = 0; i < nodeNum; i++)
    {
        ...
        ht[i].parent = ht[i].lchild = ht[i].rchild = NULL; // place empty
    ...
    {
        min1 = min2 = all_w + 10;  // set the maximal weight
        ...
        *ht[i].lchild = ht[bldbro1];
        *ht[i].rchild = ht[bldbro2];
        ht[i].weight = min1 + min2;
        *ht[bldbro1].parent = *ht[bldbro2].parent = ht[i];
        i++;
    }
...
void printHTree0(HTNode t[], int ridx)
{
    ...
    {
        if (t[ridx].weight > 0)
            printf("%3d->%5d\n", (ridx < leafNum) ? t[ridx].id : t[ridx].id + leafNum, t[ridx].weight);
...
void printHTree1(HTNode ht[])
{
    ...
    for (int i = 0; i < nodeNum; i++)
    {
        if (i < leafNum)
            ...
            printf("%2x %3d -> %5d => p:%3d lc:%3d rc:%3d\n", ht[i].id, ht[i].id + leafNum, ht[i].weight, ht[i].parent, ht[i].lchild, ht[i].rchild);
    }
}
...
unsigned char *Parsefile(const char filename[], long *w, long *fsize)
{
    ...
    *fsize = ftell(fp);
    ...
    for (int i = 0; i < *fsize; i++)
        w[i] = 0; // clear up the values of weight
    for (int i = 0; i < *fsize; i++)
...
void GetCode(HTNode ht[], int c, unsigned *code1, int *clen1)
{
    ...
    unsigned code = 0; // code

    int size = sizeof(code);             // bits for each code
    ...
    {
        code = code >> 1;
        code = (ht[parent->id].lchild->id == c) ? code : (code |= mask);
        c = parent->id;
        parent->id = ht[c].parent->id;
    }
...
{
    int sizecode = sizeof(unsigned int);
    long i = 0;
    long j = 0;
    newcode[j] = 0;
    unsigned code;
...
void zip(char fout[], HTNode ht[], int root_id, HCode hc[], unsigned char *content, long fsize)
{
    ...
    fwrite(ht, sizeof(HTNode), leafNum, fp);                // 保存哈夫曼树 ，解码要用到
...
int main()
{
    ...
    HCode hc[leafNum];  // 结构体数组，表示哈夫曼编码表 :code,len，用于编码文件
