...
/*使用数组的最后一个元素进行覆盖
既不影响degree/nv/ne/的使用，
也不需要大费周章的更改print graph函数*/
...
void deleteEdge(Graph *g, int id)
{
    for (int i = 0; i < g->ne; i++) // delete the vertex info abt the edges first
        ...
        {
            int h = g->e[i].h; // info in head vex eliminated
            for (int j = 0; j < g->v[h].degree; j++)
                if (g->v[h].edges[j] == id)
                {
                    ...
                    g->v[h].edges[j] = g->v[h].edges[g->v[h].degree - 1]; // 用最后一条边信息进行覆盖
                    g->v[h].degree--;
                }
            ...
            for (int j = 0; j < g->v[t].degree; j++)
                if (g->v[t].edges[j] == id)
                {
                    ...
                    g->v[t].edges[j] = g->v[h].edges[g->v[h].degree - 1];
                    g->v[t].degree--;
                }

            // for (int j = i; j < g->ne; j++) // info of the edge in the graph eliminated
            //     g->e[j] = g->e[j + 1];
            g->e[i] = g->e[g->ne - 1]; // 用最后一条边进行覆盖
            g->ne--;
            return;
        }
...
{ // Traverse to ensure there is no repetitive edges
    ...
    for (int h = 0; h < g->nv; h += 2)
        for (int t = h; t < g->nv; t += 2, skip = 0)
        {
            ...
            if (!skip)
                addEdge(g, rand() % 10, h, t);
        }
}

bool visited[MAXVERTEX];
void Dijstra(Graph *g, int sv) // sv: source vertex
{
    int Dist[g->nv];
    for (int i = 0; i < g->nv; i++)
        Dist[i] = INFINITY;
    ...
    for (int i = 0; i < g->nv; i++)
    {
        ...
        int min = INFINITY;
        int min_v;
        for (int v = 0; v < g->nv; v++) // Find the nearest vex
        ...
        //
        visited[min_v] = 1;
        for (int k = 0; k < g->v[min_v].degree; k++)
        {
            Edge e = g->e[g->v[min_v].edges[k]];
            int vv = (e.h == min_v) ? e.t : e.h;
            if (!visited[vv] && Dist[min_v] != INFINITY && (Dist[min_v] + e.w) < Dist[vv])
                Dist[vv] = Dist[min_v] + e.w;
        }
    }
    printf("Vex \t Distance from Vex[%d]\n", sv);
    for (int i = 0; i < g->nv; i++)
        printf("%d \t %d\n", i, Dist[i]);
}
...
void BFSTraverse(Graph *g)
{
    ...
    for (int v = 0; v < g->nv; v++)
        ...
        {
            visited[v] = 1;
            printf("Vex[%d], wieght: %d\n", g->v[v].id, g->v[v].w);
            int Qend;
            // Enqueue
            for (Qend = 0; Qvex[Qend] != INFINITY; Qend++)
                continue;
            Qvex[Qend] = v;
            //
            while (Qvex[0] != INFINITY)
            {
                // Dequeue
                int temp = Qvex[0];
                for (int i = 0; Qvex[i] != INFINITY; i++)
                    Qvex[i] = Qvex[i + 1];
                //
                for (int i = 0; i < g->v[temp].degree; i++)
                {
                    Edge e = g->e[g->v[temp].edges[i]];
                    int vv = (e.h == v) ? e.t : e.h;
                    // Locate teh next vex that is connected the present vex
                    if (!visited[vv])
                    {
                        visited[vv] = 1;
                        printf("Vex[%d], wieght: %d\n", g->v[vv].id, g->v[vv].w);
...
void DFSwithoutoutput(Graph *g, int v)
{
    visited[v] = 1;
    // printf("Vex[%d], wieght: %d\n", g->v[v].id, g->v[v].w);
    for (int i = 0; i < g->v[v].degree; i++)
    {
        Edge e = g->e[g->v[v].edges[i]];
        int vv = (e.h == v) ? e.t : e.h;
        // Locate the next vex that is connected to the present vex
        if (!visited[vv])
            DFSwithoutoutput(g, vv);
    }
}
...
void GetConnectionPieceNumber(Graph *g)
{
    int omega = 0;
    for (int i = 0; i < g->nv; i++)
        visited[i] = 0;
...
int main()
{
    ...
    for (int i = 0; i < 2; i++)                     // 链接0-1， 2-3
        addEdge(g, (i + 1) << 2, i * 2 + 1, i * 2); // graph, weight, idhead, idtail
    // printgraph(g);
    // printf("\n");
    ...
    addEdge(g, 9, 1, 3); // graph, weight, head vex id, tail vex id

    GetConnectionPieceNumber(g);
    addVex(g, 30); // graph, weight
    // GetConnectionPieceNumber(g);
    // addEdge(g, 1, 2, 5); // graph, weight, head, tail
    // GetConnectionPieceNumber(g);
    // deleteVex(g, 5); // graph, vex
    // GetConnectionPieceNumber(g);
    // printgraph(g);
    // printf("\n");
    // saveGraph(g, "deux.html");

    // getVexByEdge(g, 4, h, t); // graph, id, head vex id storage, tail vex id storage
    // printf("%d %d\n", *h, *t);
