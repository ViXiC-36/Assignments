...
Hstring *StrInit(Hstring *s, char *txt)
{
    ...
    for (i = 0; (txt[i] != 0) && (i < MAX_STRLEN); i++)
        s->str[i] = txt[i];
    if (i < MAX_STRLEN)
...
{
    if (a->length + b->length > MAX_STRLEN)
    {
        printf("String length overflowed!");
        return a;
    }
    Hstring *c = NULL;
    c = StrInit(c, a->str);
...
Hstring *StrReplace(Hstring *s, char *old, char *new)
{
    ...
    while (loc[times] != -1)
    {
        ...
        for (i = 0; i < StrLength(new); i++)
            t->str[i + loc[times]] = 1; // fill in the new parts with pseudo to pretend to be researched
